...
static cdx_int32 Resync(CdxStreamProbeDataT *p){
    ...
    do {
        ...
        for (j = 0; j < 2; ++j) {
            ...
            CDX_LOGE("subsequent header is %08x, test_pos: %lld", test_header, test_pos);
                              ...
                              cdx_int64 *postId3Pos, cdx_int32 *outHead)
{
    ...
    do {
        ...
        for (j = 0; j < SYNCFRMNUM - 1; ++j) {
            ...
            CDX_LOGD("subsequent header is %08x , header is %08x orioffset %lld test_pos %lld", test_header, header, orioffset, test_pos);
        ...
        CDX_LOGV("avgbitrate: %d, realframes: %d",avgbitrate, realframes);
