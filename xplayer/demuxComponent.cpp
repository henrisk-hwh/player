...
void DemuxCompDestroy(DemuxComp* d)
{
    ...
    //* send a quit message.
    demux->bStopping = 1;
    setMessage(&msg, DEMUX_COMMAND_QUIT, (unsigned int)&demux->semQuit);
...
void DemuxCompClear(DemuxComp* d)  //* clear the data source, like just created.
{
    ...
    //* send clear message.
    demux->bStopping = 1;
    setMessage(&msg, DEMUX_COMMAND_CLEAR, (unsigned int)&demux->semClear);
    AwMessageQueuePostMessage(demux->mq, &msg);
...
int DemuxCompCancelPrepare(DemuxComp* d)   //* should call back DEMUX_PREPARE_FINISH message.
{
    ...
    setMessage(&msg,
               ...
               (unsigned int)&demux->semCancelPrepare,  //* params[0] = &semCancelPrepare.
               0);                                      //* no reply.
    AwMessageQueuePostMessage(demux->mq, &msg);
...
int DemuxCompStart(DemuxComp* d)
{
    ...
    setMessage(&msg,
               ...
               (unsigned int)&demux->semStart,      //* params[0] = &SemStart.
               (unsigned int)&demux->nStartReply);  //* params[1] = &nStartReply.
    AwMessageQueuePostMessage(demux->mq, &msg);
...
int DemuxCompStop(DemuxComp* d)    //* close the data source, must call prepare again to restart.
{
    ...
    setMessage(&msg,
               ...
               (unsigned int)&demux->semStop,       //* params[0] = &mSemStart.
               (unsigned int)&demux->nStopReply);   //* params[1] = &mStartReply.
    AwMessageQueuePostMessage(demux->mq, &msg);
...
process_message:
        ...
        else if(msg.messageId == DEMUX_COMMAND_STOP)
        {
            ...
            {
                void* status;
                setMessage(&newMsg, DEMUX_COMMAND_QUIT, (unsigned int)&demux->semCache);
                AwMessageQueuePostMessage(demux->mqCache, &newMsg);
        ...
        else if(msg.messageId == DEMUX_COMMAND_SEEK)
        {
            ...
            if(demux->pParser != NULL)
            {
                ...
                if(demux->cacheThreadId != 0)
                {
                    setMessage(&newMsg, DEMUX_COMMAND_PAUSE, (unsigned int)&demux->semCache);
                    AwMessageQueuePostMessage(demux->mqCache, &newMsg);
                    SemTimedWait(&demux->semCache, -1);
                    setMessage(&newMsg, DEMUX_COMMAND_SEEK, (unsigned int)&demux->semCache);
                    AwMessageQueuePostMessage(demux->mqCache, &newMsg);
                    SemTimedWait(&demux->semCache, -1);
                }
                ...
                if(ret == 0)
                {
                    ...
                    if(demux->cacheThreadId != 0)
                    {
                        setMessage(&newMsg, DEMUX_COMMAND_START, (unsigned int)&demux->semCache);
                        AwMessageQueuePostMessage(demux->mqCache, &newMsg);
                        SemTimedWait(&demux->semCache, -1);
                    }
