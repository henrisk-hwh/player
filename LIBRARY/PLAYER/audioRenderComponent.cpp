...
process_message:
        ...
        {
            logd("audio render process start message.,p->eStatus = %d",p->eStatus);
            ...
            else
            {
                ...
                if(p->pSoundCtrl)
                {
                	logd("start sound device from paused status");
					int startRet = p->pSoundCtrlOps->SoundDeviceStart(p->pSoundCtrl);
                    if(startRet != 0){
						loge("start sound device fail");
						if(p->callback != NULL){
							p->callback(p->pUserData, PLAYER_AUDIO_RENDER_NOTIFY_START_FAIL, NULL);
						}
						if(pReplyValue != NULL)
                			*pReplyValue = -1;
            			if(pReplySem)
				...
				if(p->pRawSndCtrl)
		{
			//SoundDeviceStart_raw(p->pRawSndCtrl);   we have open once   dont need open pcm again
				...
				else
				{
					int startRet = p->pSoundCtrlOps->SoundDeviceStart(p->pSoundCtrl);
                    if(startRet != 0){
						loge("start sound device fail333");
						if(p->callback != NULL){
							p->callback(p->pUserData, PLAYER_AUDIO_RENDER_NOTIFY_START_FAIL, NULL);
						}
						if(pReplyValue != NULL)
