...
static cdx_int32 CdxHttpStreamingStart(CdxHttpStreamImplT *impl)
{
    ...
    do
    {
		...
		CDX_LOGD("llh>>>CdxHttpSendRequest begin");
        res = CdxHttpSendRequest(impl, impl->baseOffset);
		CDX_LOGD("llh>>>CdxHttpSendRequest finish");
        if(res < 0)
        {
		...
		CDX_LOGD("llh>>>CdxHttpReadResponse begin");
        httpHdr = CdxHttpReadResponse(impl);//--hdr->body has data.
        CDX_LOGD("llh>>>CdxHttpReadResponse finish");
        if (httpHdr == NULL)
        {
                ...
                {   /*chunked size has been force stop last time, in order to
                        ...
                        combine "len" this time. should clear when not forcestop.*/
                    {
                        ...
                        if(needReadLen >= 0)
                        {
                            ...
                            if(needReadLen == 0)
                            {
                                ...
                                impl->totalSize = impl->httpDataSize;
                                break;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if(needReadLen == 0)
                        {
                            ...
                            impl->totalSize = impl->httpDataSize;
                            CDX_LOGD("stream end. %lld", impl->totalSize); // save size
                            impl->ioState = CDX_IO_STATE_EOS;
                            break;
                        }
...
static cdx_int32 CdxSeekReconnect(CdxStreamT *stream, cdx_int64 offset)
{
    ...
    do
    {
		...
		CDX_LOGD("llh>>>CdxHttpSendRequest begin in CdxSeekReconnect");
        ret = CdxHttpSendRequest(impl, offset);
		CDX_LOGD("llh>>>CdxHttpSendRequest finish in CdxSeekReconnect");
        if(ret != 0)
        {
		...
		CDX_LOGD(" llh>>>CdxHttpReadResponse begin in CdxSeekReconnect");
        httpHdr = CdxHttpReadResponse(impl);
		CDX_LOGD(" llh>>>CdxHttpReadResponse finish in CdxSeekReconnect");
        if(httpHdr == NULL)
        {
            CDX_LOGE("Read http response failed.");
            impl->ioState = CDX_IO_STATE_ERROR;
        ...
        switch(httpHdr->statusCode)
        {
            ...
            case 206:
            {
                CDX_LOGD("llh>>>====== seek reconnect ok, url:(%s), offset:(%lld)", url->url, offset);
                break;
            }
            case 301: // Permanently
            case 302: // Temporarily
...
static cdx_int32 __CdxHttpStreamSeek(CdxStreamT *stream, cdx_int64 offset, cdx_int32 whence)
{
	...
	CDX_LOGD("llh>> in __CdxHttpStreamSeek : whence = %d",whence);
    switch(whence)
    {
    ...
    else // b. < protectAreaPos || > bufPos
    {
        ...
        //CDX_LOGV("xxx offset(%lld),validDataSize(%lld), [(%lld),(%lld)],impl->readPos(%lld)",
        //         offset, impl->validDataSize, impl->protectAreaPos, impl->bufPos,impl->readPos);
        CDX_LOGD("llh>>>in __CdxHttpStreamSeek,re_connect");
        goto re_connect;
    }
...
static cdx_int64 __CdxHttpStreamSize(CdxStreamT *stream)
{
    ...
    impl = CdxContainerOf(stream, CdxHttpStreamImplT, base);
    if(impl->totalSize == -1)
        impl->totalSize = impl->httpDataSizeChunked;
...
static int CdxGetProbeData(CdxHttpStreamImplT *impl)
{
	...
	CDX_LOGD("llh>>> in CdxGetProbeData,impl->validDataSize = %lld,impl->probeData.len = %d",
		impl->validDataSize,impl->probeData.len);
...
static cdx_int32 StreamRead(CdxHttpStreamImplT *impl, void *buf, cdx_uint32 len, cdx_bool *isEos)
{
    *isEos = 0;
    ...
    if(impl->chunkedFlag)
    {
        ...
        logd("CdxReadChunkedData %d" ,size);
        //impl->httpDataSize += size;
        return size;
    }
    else
    {
