...
static struct addrinfo *SDSNetSearch(const char *hostname, int port)
{
	...
	CDX_LOGD("llh>>>getaddrinfo begin");
    ret = getaddrinfo(hostname, strPort, NULL, &retAddr);
	CDX_LOGD("llh>>>getaddrinfo finish");
    if (ret != 0)
    {
	...
	const char *ip_addr;
	char abuf[128];
	if(retAddr->ai_family == AF_INET){
		sinp = (struct sockaddr_in*)retAddr->ai_addr;
		ip_addr = inet_ntop(AF_INET,&sinp->sin_addr,abuf,128);
		CDX_LOGD("llh>>> ip_addr = %s",ip_addr);
	}
    return retAddr;
}
...
static inline void SDSNetSearchWrap(struct SDSInstanceS *instance, CdxMessageT *msg)
{
	...
	CDX_LOGD("llh>>>add node to DAList");
    CdxMutexUnlock(&instance->listMutex);
...
static cdx_void __SDSMsgRecv(CdxHandlerItfT *itf, CdxMessageT *msg)
{
    ...
    ResponeHook hook)
{
    ...
    struct SDSInstanceS *instance = NULL;
    struct addrinfo *ai = NULL;
	CDX_LOGD("llh>>>SDSRequest:hostname = %s",hostname);
    if (hostname == NULL)
    {
	...
	CDX_LOGD("llh>>>in SDSRequest:SDSCacheSearch begin");
    ai = SDSCacheSearch(instance, hostname, port);
	CDX_LOGD("llh>>>in SDSRequest:SDSCacheSearch finish");
    if (ai)
    {
        *pAddr = ai;
        return SDS_OK;
    }
	CDX_LOGD("llh>>>in SDSRequest:ai == NULL");
