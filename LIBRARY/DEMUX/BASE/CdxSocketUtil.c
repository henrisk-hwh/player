                        ...
                        cdx_long timeoutUs, cdx_int32 *pForceStop)
{
	...
	CDX_LOGD("llh>>>connect begin");
    ret = connect(sockfd, (struct sockaddr *)addr, addrlen);
	CDX_LOGD("llh>>>connect finish");
    if (ret == 0)
    {
        //success
        return 0;
    ...
    //time out
    return -1;
    //CDX_LOGD("llh>>>in CdxSockAsynConnect,return success as default");
	//return 0;
}
                        ...
                        cdx_long timeoutUs, cdx_int32 *pForceStop)
{
    ...
    for (i = 0; i < loopTimes; i++)
    {
		...
		CDX_LOGD("llh>>>send data begin,len = %ld",len);
        while (1)
        {
            if (pForceStop && *pForceStop)
            {
            ...
            if (ret < 0)
            {
                ...
                {
                    //buffer not ready
                    CDX_LOGE("llh>>>send err: errno == EAGAIN");
                    break;
                }
                else
                {
            ...
            else // ret > 0
            {
                ...
                {
                	CDX_LOGD("llh>>>send data ok, return sendSize = %ld",sendSize);
                    return sendSize;
                }
            }
        }
		CDX_LOGD("llh>>>send data finish");
