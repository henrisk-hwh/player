...
static cdx_int32 PlaylistParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
			...
			logd("PlaylistParserPrefetch:playlistParser->child = %p",playlistParser->child);
...
cdx_int32 PlaylistParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    ...
    if(item->seqNum != playlistParser->curSeqNum)
    {
        ...
        CdxMediaInfoT mediaInfo;
		logd("PlaylistParserSeekTo:playlistParser->child = %p",playlistParser->child);
        ret = CdxParserGetMediaInfo(playlistParser->child, &mediaInfo);
        if(ret < 0)
        {
            CDX_LOGE(" CdxParserGetMediaInfo fail. ret(%d)", ret);
...
#if 1
    ret = CdxParserSeekTo(playlistParser->child, timeUs - mDurationUs);
#endif
_exit:
...
int PlaylistParserInit(CdxParserT *parser)
{
    ...
    CdxMediaInfoT mediaInfo;
	logd("playlist->mNumItems = %d",playlist->mNumItems);
    for(i = 0; i < playlist->mNumItems; i++)
    {
        ...
        if(!i)
        {
			...
			logd("PlaylistParserInit:playlistParser->child = %p",playlistParser->child);
        ...
        else
        {
            ...
            //pMediaInfo = &playlistParser->mediaInfo;
			logd("PlaylistParserInit:playlistParser->tmpChild = %p",playlistParser->tmpChild);
