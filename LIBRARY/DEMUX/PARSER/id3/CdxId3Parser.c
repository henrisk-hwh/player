...
cdx_int32 ID3PsrGetLanguageEncoding(cdx_int8* ulcomm)
{
...
cdx_int32    ID3PsrGetStringLen(cdx_int8 *StringPtr)
{
    cdx_uint32 Len = 0;
    while(*StringPtr++!=0)
    {
...
static void ID3PsrReadNunicode(cdx_int32 *FrameLen,cdx_int8** Frame, Id3ParserImplS *id3,
                               int frameTLen)
{
    cdx_int8 data;
    cdx_int32 i;
...
static void ID3PsrReadunicode(cdx_int32 *FrameLen,cdx_int8** Frame, Id3ParserImplS *id3,
                              int frameTLen)
{
    ...
    *Frame = id3->mInforBuf;//
    ...
    {
        data= (cdx_int8)ID3PsrGetBsInByte(1,id3,1);
        data1= (cdx_int8)ID3PsrGetBsInByte(1,id3,1);
...
cdx_int32    ID3PsrGeTID3_V2(Id3ParserImplS *parser)  //IF HAVE RETURN ID3 tag length
{
    ...
    if(BsVal == 0x494433/*ID3*/)  //ID3v2/file identifier   "ID3"
    {
        ...
        while((Id3v2len>=10)&&(frameinfoseg<ID3TAGNUM))
        {
            ...
            {
                cdx_int8 *GenreTemp = id3->mInforBuf;//for use AIF->mInforBuf buffer
                    ...
                    ||(ID3FRAME.UID == ID3UniqueTag[Id3Version][6]/*0x544c414e*/) /*TLAN*/)
            {
                ...
                ulLanguage_encoding = ID3PsrGetLanguageEncoding(ulcomm);
...
static int Id3Init(CdxParserT *id3_impl)
{
    ...
    if (probeData->len > impl->file_offset)
    {
		...
		CDX_LOGD("impl->child = %p,impl->stream = %p,impl->childStream = %p",impl->child,impl->stream,impl->childStream);
		...
		CDX_LOGD("ret = %d,impl->child = %p,impl->stream = %p,impl->childStream = %p",ret,impl->child,impl->stream,impl->childStream);
        if (ret == 0)
        {
...
static cdx_int32 __Id3ParserClose(CdxParserT *parser)
{
...
#endif
	CDX_LOGD("impl->child = %p,impl->childStream = %p,impl->stream = %p",impl->child,impl->childStream,impl->stream);
	if(impl->shareStreamWithChild == 0){
		if(impl->child){
	        CdxParserClose(impl->child);
			impl->child = NULL;
			impl->childStream =NULL;
	    }else if(impl->childStream){
    ...
    CdxFree(impl);    
    CDX_LOGD("return CDX_SUCCESS:impl->child = %p,impl->childStream = %p,impl->stream = %p",
    		impl->child,impl->childStream,impl->stream);
    return CDX_SUCCESS;
}
